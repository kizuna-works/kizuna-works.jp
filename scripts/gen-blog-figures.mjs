...
const FIGURES = [
  ...
  },
  {
    ...
    el: overview5([
      ...
      { n: "5", name: "祝日を除く営業日は出せない", desc: "kintoneは祝日を持っていない。土日だけを除いた営業日数なら出せる。", tag: "標準では不可", paid: true },
    ]),
  },
  {
    name: "kintone-shonin-flow-plugin-hikaku-limits",
    // 4番目の説明が2行に折り返すぶん、5番目のカードが 596px では下で切れる（旧版は切れたまま公開していた）
    w: 1000, h: 648,
    el: overview5([
      { n: "1", name: "一覧からまとめて承認できない", desc: "一覧のステータスはただの文字。1件ずつ開いて押すしかない。", tag: "標準では不可", paid: true },
      { n: "2", name: "代理承認は設定しないと止まる", desc: "作業者以外でも実行できるアクションを設定すれば代理承認できる。", tag: "標準設定で可能", paid: false },
