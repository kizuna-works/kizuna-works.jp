...
なお、**代理承認（作業者以外の人が進める）は標準でできます**。プロセス管理の設定で［作業者以外でも実行できるアクションを作成する］を選ぶと、通常のアクションとは別に、指定したユーザー／組織／グループ（または閲覧できる全員）が実行できるアクションを用意できます（[公式ヘルプ](https://jp.kintone.help/k/ja/app/process/set_non_assignee_actions.html)）。承認者の不在対策や、申請の取り戻しに使えます。設定していないステータスでは、従来どおり作業者しか進められません。
