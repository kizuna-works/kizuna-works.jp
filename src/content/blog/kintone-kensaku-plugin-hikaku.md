...
つまずくのはその先です。**キーワード検索の対象になるのは、文字列（1行）・文字列（複数行）・リッチエディター・リンク・（コピー元が文字列かリンクの）ルックアップだけ**で、**数値・日付・ドロップダウン・ラジオボタン・チェックボックスは検索されません**（[公式ヘルプ：データを検索する](https://jp.kintone.help/k/ja/search/search_details.html)）。「金額で探す」「ステータスで探す」は絞り込みを組むことになりますし、検索結果も一覧画面ではなく別画面に出ます。
