...
### テーブル（明細行）の合計は、計算フィールドで出す

「テーブルの中身が集計できない」という声はよく聞きます。ここは考え方を分けると整理できます。

kintoneのグラフは**レコード単位**の集計です。一方、[テーブル](/glossary/subtable/)は1レコードの中に複数行を持つ構造なので、「1レコード内の明細を合計する」のはグラフの仕事ではなく、**計算フィールドの役割**です。

やり方はシンプルで、**テーブルの外**に[計算フィールド](/glossary/calc/)を置き、SUM関数を指定します。

```
SUM(小計)
```

これで「合計金額」のようなフィールドに、テーブル内の「小計」の合計が表示されます。行ごとの計算（`数量*単価` で金額を出すなど）は、テーブルの**中**に計算フィールドを置けば同じように作れます。

> 合計対象に未入力の行があっても合計されます。空欄でエラーにならない点は公式ヘルプにも明記されています。

そして、この**合計フィールドをグラフの集計対象にすれば、レコードを横断した集計ができます**。「明細はテーブル、合計は計算フィールド、横断集計はグラフ」という二段構えが定石です。計算式の書き方でつまずきやすい点は[kintone計算フィールド完全ガイド](/blog/kintone-keisan-field/)にまとめています。
