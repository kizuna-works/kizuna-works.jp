---
title: "【設定ガイド】条件分岐自動採番プラグインの使い方｜部署・担当者別の連番やCSV一括採番"
description: "『条件分岐自動採番 for kintone』の使い方・設定ガイド。部署・カテゴリ・担当者などフィールドの条件で採番ルールを自動で切り替える方法を、フォーマットの組み立て・条件設定・連番リセット・CSV一括採番まで、設定画面のUIだけで完結する手順で解説します。"
pubDate: 2026-04-19
author: "KIZUNA Works"
tags: ["kintone", "プラグイン", "採番", "業務効率化", "kintoneカスタマイズ"]
ogImage: "/images/blog/conditional-numbering-plugin.png"
# 製品名「条件分岐自動採番」に検索需要がなく、内容も製品ページと重なるため
# Google 側で「クロール済み・未インデックス」となっていた。導入済みユーザー向けの
# 設定マニュアルとしては有用なので記事は残し、検索インデックスからのみ外す。
noindex: true
---

kintoneの標準機能にも「レコード番号」はありますが、実務では<strong>「伝票番号に部門コードを入れたい」「月が変わったら連番をリセットしたい」「営業所ごとに独立した採番にしたい」</strong>といったニーズが出てきます。

そのたびにカスタマイズJSで個別対応するのは、構築・保守ともに手間がかかります。

今回、こうした要件をプラグイン設定画面のUIだけで設定できる **「条件分岐自動採番プラグイン」** を開発しました。

> kintoneの採番方法の全体像（標準の限界や、無料・有料の選択肢の比較）から知りたい方は、先に [kintone自動採番を無料で実現する方法｜標準の限界とプラグイン比較](/blog/kintone-jido-saiban/) をご覧ください。**本記事は、その中で紹介した「条件分岐自動採番プラグイン」の設定手順に絞った実践ガイド**です。
