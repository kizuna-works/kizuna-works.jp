...
export const plugins: Plugin[] = [
  ...
  },
  {
    ...
    categories: ['表示・編集制御'],
    description:
      'ステータス・日付・フィールド値などの複数条件（AND/OR）を満たすと、レコードを自動でロックして編集を制限するプラグイン。レコード全体・特定フィールドに加え、ユーザーオートフィルの本人捺印もロック対象にできます。除外ユーザー・グループ・組織の設定にも対応。',
    ...
    price: 0,
    cardDescription:
      '条件を満たすレコードを自動ロック。ステータス・期日切れ・特定値などで編集制限。本人捺印のロックにも対応し、管理者は除外設定で編集可能。',
    problemTitle: '受注済みレコードの編集を防ぎたい',
    problemDesc: 'ステータスや日付などの条件で自動ロック、押した印影も触らせず、管理者のみ編集できるようにしたい',
  ...
  },
  {
    ...
    categories: ['一覧表示'],
    description:
      'テーブル（サブテーブル）の明細を一覧の行としてまとめて表示できるプラグイン。明細の列は一覧設定でテーブルを置いた位置に差し込めます。左端のキー列は固定（0〜3列）でき、ゼブラ・行番号・行ホバー、日付や選択肢の条件で行の色分けもできます。',
    ...
    price: 0,
    cardDescription:
      'テーブルの明細を一覧の行としてまとめて表示。明細の列は一覧設定の位置に差し込めます。左端列の固定・ゼブラ2色・行番号・条件付き行色分けで横長一覧も見やすく。',
    problemTitle: '一覧が見づらい・明細の中身が見比べられない',
    problemDesc: 'テーブルの明細を開くのが1レコードずつで比べられない・横スクロールでキー列が消える・状態や期限を一目で把握したい',
    problemFeatured: true,
  ...
  },
  {
    ...
    description:
      '過去に入力した値やほかのアプリの値を候補表示して選ぶだけで入力できるプラグイン。狭い入力欄は大きなダイアログで開いて全文を見ながら書けます。どちらもサブテーブル内に対応。入力順を色で誘導するガイド、未入力チェック、重複チェックも備えます。',
    ...
    price: 0,
    cardDescription:
      '過去の入力値やほかのアプリの値を候補表示して入力。狭い欄は大きなダイアログで入力（どちらもサブテーブル内対応）。入力順ガイド・未入力チェックも。',
    problemTitle: '打ち直し・入力漏れと、狭い欄の入力',
    problemDesc: '過去に入れた値を毎回手入力して表記もばらつく・明細表の狭い欄では入力中の全文が見えない・入力漏れや重複に保存後まで気づけない',
    releaseDate: '2026-06-08',
    summaryVersion: '2.1.0',
  },
  {
    ...
    description:
      '区分・ステータス・入力値や対象ユーザーの条件で、フィールドの表示/非表示・必須化・読み取り専用・自動入力・選択肢の絞り込みを切り替え。自動入力は画面を開いたときにも働きます。保存・承認・削除の前に止める・確認することもでき、モバイル対応。',
    ...
    price: 0,
    cardDescription:
      '条件でフィールドの表示/非表示・必須化・読み取り専用・自動入力・選択肢の絞り込みを制御。画面を開いたときの自動入力や、保存・承認前のチェックにも対応。',
  ...
  },
  {
    ...
    description:
      'レコード詳細にカテゴリ付きの付箋メモを自由な位置で貼り付け、貼った場所にピンを残して線で結ぶプラグイン。一覧の上部でカテゴリ別の未対応件数を集計し、一覧の行にも付箋マークと件数を表示。付箋から対象レコードへジャンプやその場で対応済みにでき、通知やコメントでは埋もれる申し送りを見える化します。',
    ...
    price: 0,
    cardDescription:
      'レコード詳細にカテゴリ付き付箋を自由配置し、貼った場所にピンを残して線で結ぶ。一覧上部で未対応件数を集計、一覧の行にも付箋マークと件数を表示。個人付箋は作成者のみ秘匿。「通知とコメントの中間」。',
    problemTitle: '申し送り・気づきが埋もれず、一覧で気づきたい',
    problemDesc: 'コメントは埋もれて一覧では気づけない・どのレコードに未対応が残っているか俯瞰できない・メモを動かすとどの項目への指摘か分からなくなる',
  ...
  },
  {
    ...
    categories: ['通知・メール'],
    description:
      'レコード値を差し込んで件名・本文・宛先を組み立て、ワンクリックでメーラーを起動するプラグイン。下書きファイル方式では表や罫線つきの本文と添付ファイルも渡せます。一覧からBCC一斉／レコードごとに1通、組織や権限での出し分けにも対応。',
    ...
    price: 0,
    cardDescription:
      'レコード値を差し込んでメーラーを起動。下書きファイル方式なら書式つき本文と添付も渡せます。BCC一斉／レコードごとに1通、組織・権限での出し分けも。',
    problemTitle: 'レコードから定型メールを毎回手作業で作っている',
    problemDesc: '宛先・会社名・担当者名をメーラーにコピペし、定型文を打ち直している・表や添付を毎回つけ直している・同じ案内を全員に送るのが手間',
    releaseDate: '2026-06-26',
    summaryVersion: '1.7.0',
  },
  {
  ...
  },
  {
    ...
    categories: ['自動入力'],
    description:
      'ユーザー選択で選んだ人の組織・役職・メール・社員番号などを対応フィールドへ自動転記。プロフィールカードや承認欄の捺印をスペースに表示し、印刷・PDFにも出力。担当者本人が押すまで印を出さない本人捺印にも対応。',
    ...
    price: 0,
    cardDescription:
      'ユーザー選択で選んだ人の組織・役職・社員番号などを自動転記。承認欄の捺印は印刷・PDFにも出力でき、本人が押すまで出さない設定も。',
    problemTitle: '担当者の部署・連絡先を毎回手入力・承認欄を手書きしている',
    problemDesc: '日報や申請で報告者＝自分の部署・役職・連絡先を毎回調べて入力し、組織は文字列でばらつき、承認欄は手書きやハンコで運用している。印刷すると承認欄が空欄のまま出てくるうえ、本人が確認したのかも分からない',
  ...
  },
  {
    ...
    categories: ['集計・計算'],
    description:
      'テーブルの行をドラッグ&ドロップと↑↓ボタンで並べ替え、値ごと複製、選んだ行をまとめて移動・削除できるプラグイン。さらにチェックを入れた行だけを合計・平均・件数・最大・最小で集計して数値フィールドへ自動入力します。行の「＋」で押した行の真上に追加、一度に表示する行数を絞る折りたたみとページ送り、列見出しソート・ヘッダー固定・行番号・既存レコードへの一括反映にも対応。',
    ...
    status: 'available',
    cardDescription:
      'チェックした行だけ、合計する。明細の行を並べ替え・複製・まとめて操作でき、選んだ行だけの合計・平均・件数を数値フィールドへ自動入力。行数の多い明細は先頭 N 行だけ表示でき、＋は押した行の真上にも追加できます。',
    problemTitle: '「選んだ行だけ」の合計が出せない',
    problemDesc:
      'kintone に SUMIF はなく、回避策は作業用の計算列を1本足して IF(CONTAINS(…)) を合計すること。その列は非表示にできず CSV にも出る。行の並べ替えもできず、明細が増えると画面が縦に伸びる一方で、新しい行は必ず下にしか足せない',
  ...
  },
  {
    ...
    categories: ['出力・帳票'],
    description:
      'レコード一覧の絞り込み結果や選択した行の添付ファイルを、フォルダ階層とファイル名を指定した ZIP でまとめてダウンロード。詳細画面では添付を縮小画像で並べ、原寸で確かめて必要な分だけ選べます。種別フィルタ・事前見積り・自動分割・中止・失敗一覧に対応。',
    ...
    status: 'available',
    cardDescription:
      '解いたら、そのまま使える形で。添付を取引先別・月別のフォルダに分けた ZIP で一括ダウンロード。詳細画面では縮小画像から必要な分だけ選べます。',
    problemTitle: '添付ファイルを、必要な分だけ取り出せない',
    problemDesc:
      'kintone に一括ダウンロードの機能はなく、レコードを開いて 1 件ずつ落とすしかない。まとめて ZIP に固める方法では、解いたあとの振り分けが残り、1 レコードに 10 点ある写真から報告書用の数枚だけを選ぶこともできない',
  ...
  },
  {
    ...
    description:
      'レコード一覧に列を1本足し、テーブル（サブテーブル）の明細件数と集計値をバッジで表示します。バッジにマウスを乗せると明細がポップアップで開き、見出しクリックで並べ替え、クリックで固定すればその場で値を直して保存できます。出す一覧も選べます。',
    ...
    status: 'available',
    cardDescription:
      '一覧に明細の件数と合計をバッジで表示。マウスを乗せれば明細がポップアップで開き、見出しクリックで並べ替え、クリックで固定すればその場で値を直して保存できます。',
    problemTitle: '明細の中身を見るために、レコードを1件ずつ開いて戻っている',
    problemDesc:
      '一覧では明細が何行あるかも合計いくらかも分からない。数字を1つ確かめるだけで詳細画面を開いて戻る往復が続き、金額の大きい行を探すにも明細を上から読むことになる',
...
export const premiumPlugins: Plugin[] = [
  {
    ...
    categories: ['出力・帳票'],
    description:
      'お手持ちのPDFや白紙の用紙にレコード値・明細をドラッグ配置して帳票化。別々のテーブルの明細を1枚に並べられ、他のアプリの帳票も取り込めます。添付へ追加保存した帳票は連番が付き、初版と修正版を見分けられます。決裁欄の印影は1クリック配置。',
    ...
    tier: 'premium',
    cardDescription:
      '手持ちの PDF に kintone のデータをドラッグ配置して帳票化。添付へ追加保存した帳票は連番が付き、初版と修正版を見分けられます。決裁欄の印影も 1 クリックです。',
    problemTitle: '既存の PDF 帳票に kintone のデータを流し込みたい／保存した帳票の初版と修正版を見分けたい',
    problemDesc: '請求書・見積書・稟議書を作り直さず 1 件ずつ印刷したい。見積を修正して出し直すと添付が同じファイル名で並び、どちらが修正版か開かないと分からない。商品明細と作業明細のように表が 2 つある帳票を 1 枚に収めたい。別のアプリで作った様式を作り直さず使い回したい。申請書の押印欄に承認者の印影と押印日を入れたい。人によって出せる帳票を変えたい。絞り込んだ一覧を 1 枚の表にして印刷したい',
  ...
  },
  {
    ...
    categories: ['集計・計算', '一覧表示'],
    description:
      'グラフ・数値カード・クロス集計表を1枚に集約し、一覧のカスタマイズビューに常設の集計ダッシュボードを描画。期間セレクタで全ウィジェットを連動再集計し、アプリ横断集計・集計キャッシュに対応。クロス集計表は行合計・列合計を個別に表示でき、ポータルには別々のボードのグラフを横並びに配置できます。',
    ...
    tier: 'premium',
    cardDescription:
      'グラフ・数値カード・クロス集計を1枚に集約。期間連動再集計・アプリ横断・集計キャッシュ対応。クロス集計は行合計・列合計を個別表示。ポータルには複数ボードのグラフを横並び常設。',
  ...
  },
  {
    ...
    description:
      'ユーザー選択・組織選択・グループ選択フィールドの候補を、所属組織・役職・他フィールドの値・ステータスで自動的に絞り込むプラグイン。準備用のアプリは不要で、所属組織ごとの出し分けは1行＝1組織の対応表で設定。候補外の値は保存時にブロックでき、サブテーブルにも対応。',
    ...
    tier: 'premium',
    cardDescription:
      'ユーザー・組織・グループ選択の候補を、所属組織や申請種別などの条件で自動的に絞り込み。準備用のアプリは不要で、所属組織ごとの出し分けは対応表で設定できます。',
    problemTitle: '担当者や申請先を、部署外まで含めた全員から選ばせたくない',
    problemDesc:
      '申請先組織や承認者を、所属組織・役職・申請種別に応じた候補だけに絞りたい。部署ごとに選べる先が違うのも設定で表現でき、候補外の人が入ったまま保存されるのも防ぎたい',
  ...
  },
  {
    ...
    description:
      'レコード詳細の関連レコード一覧を、集計・見た目・検索/ソートでまとめて強化するプラグイン。カーソルAPIで参照先を全件取得し、合計・平均・件数などの集計カード（条件付き・複数条件の並列・閾値で色分け）を表示。カードは一覧の上／下から選べ、横に並べた一覧どうしの表示位置もそろえられます。非表示フィールドやサブテーブル内の展開集計、集計結果の書き込み保存にも対応します。',
    ...
    tier: 'premium',
    cardDescription:
      '関連レコード一覧に合計・平均・件数の集計カードを表示（一覧の上／下を選択可）。横に並べた一覧の表示位置そろえ、非表示フィールド・サブテーブルの集計、書き込み保存、見た目・検索/ソートに対応。',
    problemTitle: '関連レコード一覧が「見るだけ」で集計できない',
    problemDesc: '関連レコードの合計・件数を別アプリやExcelで二度手間で出している。表示外の項目やサブテーブルも集計したい。横に並べた一覧は行がずれて見比べづらい',
    releaseDate: '2026-07-11',
    summaryVersion: '1.1.0',
  },
  {
  ...
  },
  {
    ...
    categories: ['自動入力'],
    description:
      'あらかじめ登録した「値のセット」を選ぶだけで、複数フィールドとテーブル明細をまとめて入力できるプラグイン。明細の日付・時刻も「本日」「現在日時」で持てます。管理者が作る共通テンプレートと、各ユーザーが詳細画面から作る個人テンプレートの2階層に対応。',
    ...
    tier: 'premium',
    cardDescription:
      '登録した値のセットを選ぶだけで複数フィールドとテーブル明細を一括入力。明細の日付も「本日」で持てる。管理者が配る共通と、現場が作る個人の2階層。',
  ...
  },
  {
    ...
    tier: 'premium',
    cardDescription:
      '添付PDFに押印・手書き・テキスト・図形をその場で書き加えて保存。スマホで書いたサインは、登録した署名欄（最大10か所）から置く欄を選べます。白塗りでは残る情報を焼き直して消す墨消しつき。',
