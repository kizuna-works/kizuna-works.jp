  ...
  'kw-annotation': {
    extraComm: [
      ...
      },
      {
        ...
        detail:
          '「見せる相手」に組織を指定した注釈がある画面では、閲覧している方の所属組織とその上位組織を確認します（/v1/organizations.json ほか）。「営業部」への指定を、その下の「営業一課」の方にも効かせるためです。返ってくるのは組織の名前と親子関係だけで、レコードの値は含みません。ユーザーだけを指定した注釈では、この確認は行いません。',
      },
      {
        label: '注釈を貼る対象の読み取り（自ドメイン内）',
        detail:
          '設定画面で対象を選ぶために、アプリのフォームと一覧の定義（/k/v1/app/form/fields.json・/k/v1/app/views.json）を読み取ります。また、配置モードで実際の画面を開くために、先頭レコードの番号だけを 1 件取得します（/k/v1/records.json・取得する項目は $id のみ）。レコードの値は読み取りません。',
      },
    ],
  },
};
