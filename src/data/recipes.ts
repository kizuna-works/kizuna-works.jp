...
export const recipes: Recipe[] = [
  ...
  // ============================ ポータル埋め込み ============================
  {
    ...
    goal: '作成したグラフ（ウィジェット）を、スペースや全社ポータルに常時表示して、全員が毎日同じ数字を見られるようにします。スペースのトップ・全社ポータルのいずれも、生成された JavaScript を登録するだけで表示でき、お知らせ本文の編集は不要です（プラグインがポータルAPIで iframe を自動注入します）。（プレミアム機能）',
    steps: [
      '設定画面の STEP3「ポータル埋め込み（プレミアム）」を開く',
      '「表示対象」で表示したいグラフ（ウィジェット）にチェック（複数可・チェックなし＝ボード全体）',
      '「埋め込み先」を選ぶ（スペースのトップ／全社ポータル）',
      '「埋め込みコードを生成」をクリック',
      '生成された kw-dashboard-portal.js を cybozu.com 共通管理「JavaScript / CSSでカスタマイズ（PC用）」に登録して保存（お知らせの編集は不要）',
      '【スペースに出す場合】→ お知らせの“上”（スペーストップ）に自動表示されます',
      '【全社ポータルに出す場合】→ ポータルのヘッダー直下に自動表示されます',
    ],
    tags: ['KPI'],
  },
];
