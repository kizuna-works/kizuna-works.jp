...
export const glossary: Term[] = [
  ...
  },
  {
    ...
    seoTitle: 'kintoneの一覧画面とは？ビューの設定と見やすくする方法',
    seoDescription: 'kintoneの一覧画面とは、レコードを表形式で表示する起点となる画面のこと。ビューによる絞り込み・並び替えの設定方法と、検索窓の追加や列固定など一覧を見やすく改善するコツまでわかりやすく解説します。',
  ...
  },
  {
    ...
    seoTitle: 'kintoneの計算フィールドとは？数式の書き方と集計の限界',
    seoDescription: 'kintoneの計算フィールドとは、他フィールドの値から数式で自動計算する読み取り専用フィールド。単価×数量やIF条件分岐の書き方と、一覧での合計・レコード横断集計ができない場合の対処法まで解説します。',
    relatedTerms: ['number'],
  ...
  },
  {
    ...
    seoTitle: 'kintoneのプロセス管理とは？ワークフロー設定とステータス活用の基本',
    seoDescription: 'kintoneのプロセス管理とは、ステータスで承認・作業フローをノーコード構築できる標準機能。未対応→対応中→完了の設定手順と、作業者・アクションの決め方、一覧からの一括処理まで実務目線で解説します。',
    relatedTerms: ['status'],
  ...
  },
  {
    ...
    seoTitle: 'ルックアップとは？kintoneでの意味と関連レコード一覧との違い',
    seoDescription: 'ルックアップとは、kintoneで別アプリのマスタから値をコピーして入力を省力化するフィールド機能のこと。取得した値が自動追随しない仕組みと、関連レコード一覧との違いを最短で理解できるようまとめました。',
    relatedTerms: ['related-records', 'field'],
    relatedPlugins: ['lookup-suggest', 'lookup-sync'],
    relatedBlog: ['kintone-lookup-kaisetsu', 'kintone-2026-04-update'],
    hasPage: true,
    longDescription: [
      ...
      'フィールドの作り方・コピー元の指定・絞り込み条件・テーブル内での使い方といった設定手順は、ブログ記事「kintoneのルックアップの使い方と設定方法」で画面に沿って解説しています。このページは用語の意味と考え方に絞ってまとめています。',
    ],
    useCases: [
  ...
  // ----- システムフィールド -----
  {
    ...
    seoTitle: 'kintoneのレコード番号とは？リセット・変更の可否と年度別連番の作り方',
    seoDescription: 'kintoneのレコード番号はリセットも変更もできません。削除したレコードの番号は欠番のまま戻りません。その仕様と理由、プレフィックスや開始番号として設定できる範囲を整理したうえで、年度・月でリセットする連番や区分ごとの独立連番を実現する方法まで解説します。',
    relatedTerms: ['record'],
    ...
    faq: [
      { q: 'レコード番号を1から振り直す（リセットする）ことはできますか？', a: '既存レコードの番号を振り直すことはできません。レコード番号はシステムが自動採番する読み取り専用の値だからです。アプリ設定で指定できる「開始番号」は、以降に追加されるレコードの採番に使われるもので、すでにあるレコードの番号は変わりません。年度や月で番号をリセットしたい場合は、レコード番号そのものではなく、採番プラグインで別のフィールドに採番コードを持たせる方法が現実的です。' },
