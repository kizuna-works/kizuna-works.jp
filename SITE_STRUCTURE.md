...
│   │   │   ├── list-styler/
│   │   │   │   └── index.astro # 一覧スタイラープラグイン製品ページ（/plugins/list-styler/）— テーブル展開表示（サブテーブルの列を一覧の列として追加し明細を一覧の行として表示・v1.1.0〜）＋一覧の左端列固定（0〜3）＋基本行スタイル（ゼブラ2色/単色/文字色）＋行ホバー＋行番号＋条件付き行スタイル（背景/文字色/太字・日付TODAY/選択肢/ステータス・優先ドラッグ）＋行クリックで詳細を開く・設定セット方式・kintone完結。旧 kw-fixed-columns（列固定ビュー）から改称
│   │   │   ├── input-assist/
...
│   │   ├── kw-list-styler-v1.1.0.zip                             # 一覧スタイラー for kintone 配布ファイル（最新・v1.1.0 テーブル展開表示を追加＋横スクロールを一覧内に限定）
...
│   │   ├── kw-file-export-v1.0.0.zip                             # 添付ファイル出力 for kintone 配布ファイル（手動配置・初版）
