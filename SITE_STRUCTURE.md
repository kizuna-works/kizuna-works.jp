...
│   │   │   ├── drive-connect/
│   │   │   │   └── index.astro # 【プレミアムプラグイン】ドライブ連携 製品ページ（/plugins/drive-connect/）— レコードの添付ファイルを Google ドライブの指定フォルダへ転送し、レコードにはリンクだけを残す。転送に成功したファイルは kintone 側から削除でき、ディスク使用量を抑えられる。レコード単位のフォルダはフィールドと固定文字のパーツ方式で自動生成（空フィールドは前後の区切り文字ごと省略・使用不可文字は自動置換・ID で追跡）。ファイル名＋サイズの突合による差分検知で二重転送を防止。実行モードは手動／自動／自動（条件つき・最大5件・AND/OR）で、条件は一覧からの一括転送でも尊重。サブテーブル内の添付は行ごとに処理。共有ドライブ対応。ファイルはブラウザから Google へ直送（中継サーバー不要）、要求スコープは drive.file のみ。ヒーロー＋準備中のお知らせ＋課題3＋転送2枚＋フォルダ自動生成1枚＋差分検知/削除の4手順＋実行タイミング3＋設定1枚と手順5＋主な機能9＋プラン（比較表）＋注意11＋FAQ10＋SecurityBox＋ShareButtons＋CTA。無料は転送ルール1本＋手動モードのみ、自動転送と一括転送と本数無制限はプレミアム。**現在は公開準備中（status: coming-soon）**。ゲストスペース対応
│   │   │   │   └── google-cloud/
