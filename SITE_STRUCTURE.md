...
│   │   │   ├── annotation/
│   │   │   │   └── index.astro # 【プレミアムプラグイン】注釈アシスト 製品ページ（/plugins/annotation/）※**配布準備中**（ページは公開・ダウンロードは停止。`plugins.ts` の status は coming-soon、配布 zip は `public/downloads/` に置いていない）— kintone のラベルフィールドでは置けない場所（標準ボタンの隣・画面の上部・ツールバー・ステータスバー・一覧の列見出し・テーブルの見出し・ページ送りの付近・画面に出ている文字の近く・フィールドの上下左右）へ、説明/注意書き/リンク/画像のカードを重ねて表示する。位置は設定画面から本物のレコード画面・一覧画面を配置モードで開き、置きたい場所をクリックして決める（近くの項目・見出し・文字を目印として覚えるので、フォームの並びを変えても追従する／フォームの編集は不要）。見せ方は 常に表示する／アイコンにマウスを乗せたとき／アイコンをクリックしたとき の3つから注釈ごとに選択。**重ねて描くため真下の操作部品は押せなくなるので、プロセス管理のボタン・ツールバー・［保存］・コメント欄・ページ送り等を覆うと配置モードが警告する**（赤い輪＋文言・保存は止めない）。閲覧者はカードを移動・リサイズでき、位置は保存しない（レコードを閉じると設定どおりに戻る）。見た目は配色プリセット12種類（情報・注意・警告・手順・メモ・強調・完了・質問・ふせん・黒板・ホワイトボード・ダーク）と形状10種類、色・枠線・影・幅・高さ・アイコン・見出しラベルの個別指定。**リンク色は既定が自動**（背景に合わせて 4.5:1 以上になる色を選ぶ。黒板・ダークでも読める）。［見た目の共通設定］は新しい注釈のひな形で、すでにある注釈は変わらない（［この見た目をすべての注釈に適用する］で明示的に揃える）。本文はツールバー付きエディター（太字・下線・箇条書き・番号リスト・リンク／HTML 編集も可・3000字）。注釈ごとに言語を追加すると閲覧者の kintone の表示言語で本文・リンク・画像を出し分け（8言語・未登録言語は既定言語を出す／出さないを選択）。表示する相手をユーザー・組織・グループで限定、印刷画面への表示も選択。設定画面は左に実際の見え方のライブプレビュー。ヒーロー＋課題3＋配置モード2枚＋貼れる場所の表＋見せ方3カード（スクショ3枚）＋一覧1枚＋多言語の日英対比2枚＋見た目1枚と選べるものの表＋設定3枚＋インストール手順5＋プラン（無料/プレミアム）＋できないこと6＋FAQ8＋SecurityBox＋ShareButtons＋CTA。無料は注釈1件・既定言語のみ、複数注釈と多言語出し分けと JSON 入出力がプレミアム。モバイル対応（画面上部にまとめて表示）・ゲストスペース対応
│   │   │   ├── lookup-filter/
...
│   │   ├── kw-annotation-v1.0.0.zip    # 注釈アシスト v1.0.0 配布物（最新・2026-09-22）
