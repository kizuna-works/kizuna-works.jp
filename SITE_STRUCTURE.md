...
│   │   │   ├── list-styler/
│   │   │   │   └── index.astro # 一覧スタイラープラグイン製品ページ（/plugins/list-styler/）— テーブル展開表示（サブテーブルの列を一覧の列として追加し明細を一覧の行として表示・v1.1.0〜／明細の列を出す位置＝一覧設定でテーブルを置いた位置か一覧の右端を選択・v1.2.0〜）＋一覧の左端列固定（0〜3）＋基本行スタイル（ゼブラ2色/単色/文字色）＋行ホバー＋行番号＋条件付き行スタイル（背景/文字色/太字・日付TODAY/選択肢/ステータス・優先ドラッグ）＋行クリックで詳細を開く・設定セット方式・kintone完結。旧 kw-fixed-columns（列固定ビュー）から改称
│   │   │   ├── input-assist/
...
│   │   ├── kw-list-styler-v1.2.0.zip                             # 一覧スタイラー for kintone 配布ファイル（最新・v1.2.0 明細の列を一覧設定の位置に差し込む＋左端列固定の数え方を修正）
