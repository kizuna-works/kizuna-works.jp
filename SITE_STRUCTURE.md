...
│   │   ├── plugins.ts          # プラグイン一覧の単一情報源（グリッド・フォーム・JSON-LDの元）。`categories` は目的カテゴリを1〜3個持つ配列。先頭にそのプラグインの主目的を書くが、チップはすべて同じ体裁でカテゴリ色だけが変わる（並び順に表示上の意味は持たせていない）。`bornFrom: 'supporter-request'` はサポーターのリクエストで**新規開発した**製品に付けるフラグで、一覧カード／トップのショーケース／製品ページのバッジ（RequestOriginBadge）と /plugins/・/plugins/supporter/ の件数がすべてこの1か所から出る
...
│   │   │   ├── order/
│   │   │   │   └── index.astro # ライセンス申込フォーム（/partner/order/）― 代理店コード＋申込キーで認証し、プレミアム年間サポーターライセンスを申し込む。11項目（コード／キー／顧客名／顧客ドメイン／管理番号／更新申込チェック／発行日（当日〜90日先）／請求月（自動表示）／担当者名／担当者メール／ご連絡事項）。更新にチェックすると発行日は入力不可（当社が有効期限の翌日を採る）。代理店コード・担当者名・メールはブラウザに保存、申込キーはチェックしたときだけ保存。GAS の handlePartnerOrder が受け、代理店_受付へ記録して受付／保留を返す（台帳へは承諾時にのみ転記）
│   │   │   └── setup/
...
│   │   │   ├── field-styler/
│   │   │   │   └── index.astro # フィールドスタイラー製品ページ（/plugins/field-styler/） — 背景色・フォント・文字色・文字数制限・太字・条件付きスタイル（最大10条件）・フォントサイズ・コピー・入力文字種制限・プレースホルダー・最大行数の全11機能。v2.2.0＝サブテーブル（明細）内フィールドのスタイル＋行ごとの条件付き色分け。v2.3.0＝条件ごとに背景色/文字色を「変更しない」に。**v2.4.0＝テーブル外のフィールドも作成・編集画面に反映**（入力欄の中の文字にサイズ・色・太字・フォントが効く／条件付きも入力中に追従）＋文字数制限・入力文字種制限が入力中にも動作＋ゲストスペースで設定画面のフィールド候補が出ない不具合を修正。**v2.5.0＝文字数制限・入力文字種制限をサブテーブル（明細）内の列にも設定可能に**（超過した行にエラーを出して保存を止める）＋文字列複数行に「最大行数」を新設（上限で改行できなくなる／貼り付け超過は注意書き＋保存時に停止）＋フィールドを作り直して型が変わったときの追従を明細にも拡張（UPDATEボックス＋スクショ field-styler-v250-*＝掲載用の「案件管理」アプリで撮影）
...
│   │   │   ├── image-compress/
│   │   │   │   └── index.astro # 【ちょこっと】画像圧縮 製品ページ（/plugins/image-compress/）— **レコードを保存した直後に、添付された画像を裏で圧縮して差し替える**。管理者が強さを一度決めれば**利用者の操作は何も変わらない**（専用の添付ボタンも毎回の選択もない）。強さは**軽量1280/70・標準1600/80・高画質2048/85・自分で決める の4つ**で、「自分で決める」は長辺＋**仕上げ方（品質で決める／1枚あたりの容量で決める）**の入れ子。容量指定では品質40〜95で目標に収まる最大品質を二分探索し、**長辺に応じた入力の下限**と写真での目安をその場で示す。**サブテーブル内の添付も対象**（行や他列の値は変わらない）。**ファイル名・添付の並び順は変わらず**、撮影時の向きは画像へ反映するので縦写真が横倒しにならない。長辺が設定値以下の画像・GIF・PDF・動画は対象外で、**何度保存しても劣化が積み重ならない**。登録済みレコードは**設定画面から一括圧縮**（対象＝すべて／レコード番号の範囲、［対象を確認する］の見積り＝上限であることを明示、実行前の確認モーダルで取り消せないこと・更新者・条件通知・変更履歴への影響を警告、1件ずつ直列＋進捗、いつでも中止、失敗は3回リトライ）。PCとモバイルで同じ強さが効き、**ブラウザによる書き出し品質の差は実効品質を測って自動補正**。圧縮は利用者のブラウザ内だけで完結し画像を外部送信しない。**注意＝圧縮は不可逆／条件通知が飛ぶ（未圧縮レコードを誰かが開くたび）／変更履歴を記録していると圧縮前の原本が残りディスクが減らない／Exif削除／透過なしPNGは.jpgへ**。キャッチコピー「添付写真を自動で圧縮。サブテーブルも、溜まった分もまとめて。」。ヒーロー＋課題3＋圧縮前後の比較（1枚）＋通知バナー（1枚）＋サブテーブル（1枚）＋一括圧縮3枚＋設定画面2枚＋主な機能10＋手順5＋プラン＋注意16＋SecurityBox＋FAQ15＋SupporterCTA＋ShareButtons＋関連3。ゲストスペース対応・モバイル対応。撮影は kintone 公式テンプレート「日報」＋ダミーデータ（実測 39MB → 2.7MB・93%削減）
│   │   │   ├── table-preview/
│   │   │   │   └── index.astro # 【ちょこっと】テーブルプレビュー 製品ページ（/plugins/table-preview/）— **レコード一覧に自前の列を1本足し、テーブル（サブテーブル）の明細件数と集計値をバッジで表示**。「5 ¥43,240」のように件数と合計が並び、**0行のレコードは淡色の「—」**で入っていないことも分かる。**マウスを乗せると明細がポップアップ**で開き（見せる列・並び順・列幅はテーブルごとに指定）、**クリックで固定→［編集］で既存行の値をその場で編集して保存**（固定と編集を分けてあるのは、ページ送りやタブ切替で意図せず編集に入らないため）。複数テーブルは**タブで切替**（件数つき・切替で再取得しない・下に全タブの合計）。**列の見出しクリックで明細を並べ替え**（昇順→降順→元の並び・並べ替えボタンは常時表示・順序が決まらない型は対象外・並ぶのは表示だけ）、**下に「1–20 / 25 行」で出ている行数と合計**。**開いている間は背後の一覧が動かない**（上でホイール＝明細だけ／外＝閉じて一覧が動く／編集中は動かない）。集計は件数/合計/平均/最大/最小＋前後の文字。**編集は既存行の値のみ**（行の追加・削除・並べ替えはしない）、計算・ルックアップのコピー先・添付は編集不可、空行があると保存をブロック、**保存は kintone 標準の更新**なのでアクセス権はそのまま有効・リビジョンで同時更新を検知。**行の高さは変わらない**（kintone と同じ余白の箱でバッジを描くので1行目の文字と高さがそろう／実測49px）。ただし**一覧表は画面幅いっぱいに広がるため列を足すと横スクロールが出ることがある**（列幅80〜400px）。**バッジと列を出す一覧（ビュー）は設定で選べる**（未選択＝すべての一覧・v1.1.0）。バッジの置き場所は**専用の列（既定）／詳細アイコンの隣／指定したフィールドの列**。キャッチコピー「一覧のまま、明細が見える。直せる。」。ヒーロー＋**UPDATE ボックス（v1.1.0・スクショ2枚）**＋課題3＋一覧（1枚）＋ポップアップ（2枚）＋編集（1枚）＋設定画面3枚＋主な機能13＋手順5＋プラン＋注意13＋SecurityBox＋FAQ10＋SupporterCTA＋ShareButtons＋関連3。PC専用（モバイルは何も描画しない）・ゲストスペース対応
│   │   │   ├── split-view/
...
│   │   │   ├── input-assist/
│   │   │   │   └── index.astro # 入力アシストプラグイン製品ページ（/plugins/input-assist/）— v2.0.0 で入力サジェスト（同じフィールドの過去値／ほかのアプリの指定フィールドの値を候補表示して入力・サブテーブル内の列も対応・よく使われている順/新しい順/昇順・件数指定・別項目の併記・絞り込み条件・自分の値だけ・まとめて取得/入力ごとに検索）を主機能に追加。ほかに入力順ガイド（待機/現在地/入力済みを色分け・値が入ると次へ自動前進・ラベル横にタグ・自動スクロール/フォーカス）＋未入力チェック（保存は止めず保存時に確認・詳細画面でも未入力表示）＋入力中の重複チェック（「値の重複を禁止する」フィールドを自動対象）。配色はガイド/未入力チェックで別プリセット・kintone完結。**v2.1.0＝拡大入力**（狭い入力欄を大きなダイアログで開いて入力し、確定すると元のフィールドへ書き戻す。サブテーブル内の列も対応・見出しに行番号・Tab で確定して次の対象へ連鎖・Esc で取り消し・暗い部分クリック/確定ボタン/Ctrl+Enter で確定・開き方はフィールドごとに「入力欄に入ったとき」/「拡大ボタンを押したとき」・入力順ガイドと併用すると順番が来た欄で自動的に開く）＋設定画面のカードを折り畳み式に＋当社製品を多数同居させた環境で入力サジェストと重複チェックが起動しない不具合を修正（UPDATEボックス＋スクショ input-assist-zoom-01/02＝掲載用アプリ「案件管理」で撮影）
│   │   │   ├── conditional-form/
│   │   │   │   └── index.astro # 条件分岐フォームプラグイン製品ページ（/plugins/conditional-form/）— 区分・ステータス・入力値などの条件（最大5件・AND/OR）でフィールド/グループを表示/非表示・必須化・読み取り専用・値の自動セット（TODAY対応）・選択肢の絞り込みに切替。1ルール複数動作＋優先順位ドラッグ＋安全則（非表示＞読取＞必須）＋ハイライト/必須バッジ・新規/編集/詳細をルール単位で指定・kintone完結。v2.1.0＝「が次のいずれかに一致する／いずれにも一致しない」で複数値を条件1行指定・恒真/矛盾条件の警告＋保存ブロック・設定画面のルール折りたたみ（UPDATEボックス＋スクショ conditional-form-config-05/06＝検証環境「案件管理」で撮影）。v2.2.0＝kintone 2026年8月アップデート（ルックアップの change イベント）対応：ルックアップを条件にした制御・取得/クリア直後の途中状態では条件を判定しない・条件成立時の値セット/読み取り専用が入力操作中も反映。**v2.3.0＝「実行時チェック」タブを新設**：保存前／プロセスアクション実行前／削除前に条件を判定し、止める・確認する・入力を促す（アクション実行前限定・空欄をモーダルで埋めさせ、値の書き戻しとステータス遷移が1回で完了）。条件を満たすまでアクションボタン自体を出さない予防型の出し分け＋アクション名/遷移先ステータス条件。**v2.4.0＝「値をセットする」の拡張**：固定値に加えて**同じ型のほかのフィールドの値をコピー**（例：申請者→発注担当・コピー元が空なら対象も空）、固定値は**ユーザー選択/組織選択/グループ選択は候補一覧から複数選択・チェックボックス/複数選択は選択肢を複数チェック**。条件・動作の一覧でグループ内フィールドを**「項目名（グループ名 グループ内）」**と表示（従来もグループ内は選べたが見分けられなかった）＋グループ名がフィールドコードで表示される不具合を修正（UPDATEボックス＋スクショ conditional-form-setvalue-03/04＝検証アプリ「kw-conditional-form グループ検証」で撮影）。**v2.5.0＝モバイル対応**：core.js（旧 desktop.js）を manifest の desktop / mobile 両方に登録し、端末差（イベント名・レコード API・フォームの DOM）だけを切り替え。モバイルでもフォーム制御と実行時チェック・アクションボタンの出し分けが働く。**一覧画面からの削除前チェックはモバイル非対応**（イベントが存在しない）／詳細画面での中止は画面上部エラーが使えないため自前ダイアログで理由表示。あわせて**チェックボックス・ラジオボタンの選択肢の絞り込みが効かない既存不具合を修正**（UPDATEボックスを v2.5.0 の1件に統一し、旧版の内容とスクショ conditional-form-config-07/08・prompt-01・setvalue-03 は「主な機能」「設定画面の使い方⑦〜⑩」へ内包。新スクショ conditional-form-mobile-01＝掲載用に「稟議申請」アプリのモバイル画面2枚を横並びに合成）。**v2.6.0（2026-09-24）**＝値のセットを条件を満たした状態で編集・新規作成画面を開いたときにも実行（動作ごとに「空欄のときだけ入れる（既定）／入力済みでも上書きする」）・「適用する画面」で詳細を選ぶと注意書き（UPDATE ボックスを v2.6.0 の1件に差し替え＝csv-export と同じ標準形に統一。v2.5.0 のモバイル画面 mobile-01 は「設定画面の使い方⑫」へ移動、新スクショ setvalue-05/07 は UPDATE ボックス、setvalue-06 は使い方⑪）
│   │   │   ├── sheet-edit/
│   │   │   │   └── index.astro # シート編集プラグイン製品ページ（/plugins/sheet-edit/）— レコード一覧（カスタマイズビュー）を表計算風グリッドに置換し表示中フィールドをその場編集。v1.1.0でサブテーブル（明細）の行展開・行ごと編集に対応（明細外セルは結合/空欄を選択）。型別セル編集／保存3経路（自動=行コミット・ボタン・Ctrl+S）／単一セルコピペ・範囲一括編集・縦オートフィル／Undo・Redo（操作グループ単位）／revision競合検知＋行×セル失敗ハイライト／シートタブ式ページネーション（カーソルAPIで1万件超）／型別フィルタ（選択肢複数チェック・数値範囲・部分一致）＋複数列AND・並べ替え／カラー・タブ形状カスタム／Excel出力（表示列そのまま・絞り込み後全件・サブテーブルは行展開）・kintone完結。v1.4.0で組織/ユーザー/グループ選択の絞り込みを名前検索・複数選択の選択式に＋設定セットの複製・アコーディオン表示を追加。**v1.5.0で連番・日付オートフィル**（同一列2セル以上を選んでドラッグ＝規則の続きを生成／1セルは従来コピー）＋既存不具合修正（Excel出力の明細欠落・数値の桁区切り無効・編集終了時の未捕捉例外・選択色がゼブラと同色・ホバーが選択表示を上書き）＋SheetJSを同梱から自社libs遅延読込へ。**v1.5.2でチェックボックス・複数選択セルの確定用 OK ボタンが見えなかったのを修正**（body直下のポップアップにテーマ色のCSS変数が届かず背景が透明化／外側クリックでも確定・Escで取り消しに対応／`.kw-se` を isolation:isolate にしてグリッド内部のz-indexを封じ込め、ポップアップは9000番台へ）
│   │   │   ├── sticky-board/
│   │   │   │   └── index.astro # 付箋ボードプラグイン製品ページ（/plugins/sticky-board/）— レコード詳細にカテゴリ付き付箋をクリックで自由配置（移動/リサイズ/本文編集・タイトル濃色/本文薄色・パステル10色＋カスタム）＋一覧上部に公開/個人で分けた未対応件数バー＋カード展開（レコードへ別タブ・対応済み/未対応に戻す・作成者/対応者と日時）。個人付箋はレコードACLで作成者のみ秘匿。対応済みの自動削除（既定30日・公開は全員ぶん）。保管アプリを設定画面からワンクリック自動生成。v1.1.0 で貼った場所に残る「ピン」と線（形6種・色を選択・クリックで元の位置へ）／一覧の行に付箋マークと件数（アイコン欄・値の前後から選択）／付箋カードにレコードの項目をタグ表示を追加。スクショはライトボックスで拡大・kintone完結
│   │   │   ├── comment-control/
...
│   │   │   ├── mail-assist/
│   │   │   │   └── index.astro # メールアシストプラグイン製品ページ（/plugins/mail-assist/）— レコード値を差し込んで件名・本文・宛先を組み立て、ワンクリックでメーラー（既定メーラー/Gmail/Outlook（個人・法人 Microsoft 365）/Yahoo!）を起動、または下書きファイル（.eml）を作成。下書きファイル方式は書式つき本文（表・罫線・文字色）と添付ファイルフィールドをそのまま載せられる（要デスクトップメールソフト）。詳細＝個別／一覧＝絞り込み結果をBCC一斉（最大宛先件数で分割）またはレコードごとに1通。宛先は文字列/リンク/ユーザー選択＋直接入力。テンプレごとの表示・送信条件（最大8件AND/OR・レコード値＋操作している人の組織/グループ/ユーザー/アプリ管理権限）で出し分け、メーラーは都度選択/管理者固定。送信履歴を保管アプリに記録＋詳細画面表示。テンプレ・履歴の保管アプリを同一スペースに自動生成。実送信はメーラー委譲で外部サーバー不要・kintone完結。送信方法ごとの対応表を製品ページと設定画面に掲載。スクショはライトボックスで拡大
│   │   │   ├── user-autofill/
...
│   │   │   ├── reuse-control/
│   │   │   │   └── index.astro # レコード再利用コントロール製品ページ（/plugins/reuse-control/）— kintone 標準の「レコードを再利用する」で、**「残す（コピーする）」項目だけをホワイトリストで指定し、それ以外は再利用画面の表示と同時に自動で空欄化**する。サブテーブルは**列単位**で残す／消すを選べ、行はそのまま引き継ぐ。**ルックアップは別のキーで取り直せる**（新しいキーで取得し直してコピー先も入れ替える）。フィールドの**初期値もまとめてクリア**でき、「標準では全項目がコピーされ、要らない項目を毎回手で消す」手間が無くなる。**通常の新規作成（＝再利用でない追加）には一切影響しない**。kintone 完結・外部 API 不使用。ヒーロー＋課題＋使い方＋できること7＋初期値クリア＋ルックアップ入れ替え＋設定画面＋手順＋プラン＋注意＋SecurityBox＋FAQ＋SupporterCTA＋ShareButtons＋関連
│   │   │   ├── accordion-tab/
...
│   │   │   ├── report-designer/
│   │   │   │   └── index.astro # 【プレミアムプラグイン】帳票デザイナー製品ページ（/plugins/report-designer/）— 手持ちPDFを背景に kintone データ（フィールド/固定テキスト/画像（ロゴ・印影）/サブテーブル明細）をドラッグ配置して帳票化。自動繰り返し＋自動改ページ＋要素ごとの表示ページ／日付自動入力・表示条件・PDF保存先・集計表紙／1件印刷＋一覧まとめ印刷（最大100件）＋PDF自動保存。設定ガイドをアコーディオン組込。**v1.5.0＝添付へ追加保存した帳票PDFのファイル名に連番（-1／(1)）・ファイル名の時刻トークン・保存先が削除/型変更なら理由を表示・まとめ印刷中に足された添付を落とさない**（UPDATEボックス＋スクショ report-designer-record-01/config-18）。単品販売なし＝プレミアム年間サポーターに含む（CTAは /plugins/supporter/?plan=all）。kintone完結
│   │   │   ├── dashboard/
...
│   │   │   ├── input-template/
│   │   │   │   └── index.astro # 【プレミアムプラグイン】入力テンプレート製品ページ（/plugins/input-template/）— あらかじめ登録した「値のセット（テンプレート）」を選ぶだけで複数フィールド＋テーブル明細を一括入力。管理者が設定画面で値を直接定義する共通テンプレート（実データ不要・型別の入力UI・テーブルは行追加）と、各ユーザーが詳細画面の「テンプレートにする」から必要項目だけ選んで作る個人テンプレート（本人にだけ表示）の2階層。新規作成は常に上書き、編集画面は既定オフ＋「空欄のみ/すべて」「追加/置換」を選択し失われる値だけ確認ダイアログ。適用は値をセットするだけで保存しない。テンプレートは同一スペースの保管アプリへ分離＝フォーム不変・登録数上限なし。計算/添付/重複禁止/ルックアップのコピー先は理由つきで自動除外。ヒーロー＋UPDATE(v1.2.0 ログインユーザー・優先する組織・空欄・本日から○日後)＋定義＋実画面(ボタン/モーダル/適用前後)＋課題3＋2階層＋主な機能8＋編集画面＋モバイル＋設定画面＋プラン(無料は共通5件/個人1件・機能の中身は無制限)＋手順＋注意＋SecurityBox＋ShareButtons＋FAQ7＋関連ブログ＋CTA。単品販売なし＝プレミアム年間サポーターに含む。PC・モバイル対応・kintone完結
│   │   │   ├── text-join/
...
│   │   │   ├── annotation/
│   │   │   │   └── index.astro # 【プレミアムプラグイン】注釈アシスト 製品ページ（/plugins/annotation/）※2026-09-25 配布開始（v1.0.0）— kintone のラベルフィールドでは置けない場所（標準ボタンの隣・画面の上部・ツールバー・ステータスバー・一覧の列見出し・テーブルの見出し・ページ送りの付近・画面に出ている文字の近く・フィールドの上下左右）へ、説明/注意書き/リンク/画像のカードを重ねて表示する。位置は設定画面から本物のレコード画面・一覧画面を配置モードで開き、置きたい場所をクリックして決める（近くの項目・見出し・文字を目印として覚えるので、フォームの並びを変えても追従する／フォームの編集は不要）。見せ方は 常に表示する／アイコンにマウスを乗せたとき／アイコンをクリックしたとき の3つから注釈ごとに選択。**重ねて描くため真下の操作部品は押せなくなるので、プロセス管理のボタン・ツールバー・［保存］・コメント欄・ページ送り等を覆うと配置モードが警告する**（赤い輪＋文言・保存は止めない）。閲覧者はカードを移動・リサイズでき、位置は保存しない（レコードを閉じると設定どおりに戻る）。見た目は配色プリセット12種類（情報・注意・警告・手順・メモ・強調・完了・質問・ふせん・黒板・ホワイトボード・ダーク）と形状10種類、色・枠線・影・幅・高さ・アイコン・見出しラベルの個別指定。**リンク色は既定が自動**（背景に合わせて 4.5:1 以上になる色を選ぶ。黒板・ダークでも読める）。［見た目の共通設定］は新しい注釈のひな形で、すでにある注釈は変わらない（［この見た目をすべての注釈に適用する］で明示的に揃える）。本文はツールバー付きエディター（太字・下線・箇条書き・番号リスト・リンク／HTML 編集も可・3000字）。注釈ごとに言語を追加すると閲覧者の kintone の表示言語で本文・リンク・画像を出し分け（8言語・未登録言語は既定言語を出す／出さないを選択）。表示する相手をユーザー・組織・グループで限定、印刷画面への表示も選択。**表示する条件**（レコードの値・見ている人〔所属組織・アプリ管理者か・編集できるか・作業者か〕・画面〔パソコン/モバイル〕・日時の組み合わせ）で出し分け、注釈ごとに［ヘルプ］をオンにすると画面の［ヘルプ］ボタンを押した人にだけ表示（オン/オフは利用者ごとに記憶・ボタンの色と形は選択可）。注釈の複製、別の画面への移動も可。設定画面は左に実際の見え方のライブプレビュー。ヒーロー＋課題3＋配置モード2枚＋貼れる場所の表＋見せ方3カード（スクショ3枚）＋一覧1枚＋多言語の日英対比2枚＋見た目1枚と選べるものの表＋設定3枚＋インストール手順5＋プラン（無料/プレミアム）＋できないこと6＋FAQ8＋SecurityBox＋ShareButtons＋CTA。無料は注釈1件・既定言語のみ、複数注釈と多言語出し分けと JSON 入出力がプレミアム。モバイル対応（画面上部にまとめて表示）・ゲストスペース対応
│   │   │   ├── lookup-filter/
│   │   │   │   └── index.astro # 【プレミアムプラグイン】ルックアップ絞り込み 製品ページ（/plugins/lookup-filter/）— ルックアップの［取得］で開く一覧を、入力中のレコードの値に応じて絞り込む。kintone 標準の「絞り込みの初期設定」は固定値しか書けない隙間を埋め、条件の右辺に 入力中のレコードのフィールド／操作している人（ログインユーザー・優先組織）／実行するときの日付／固定値／見る人ごとの対応表 を指定できる。絞り込みは参照先アプリへの問い合わせ条件として送る（取得後に隠すのではない）。場面ごとの条件セットは上から順に判定し最初の1つだけを使う。保存時ガード＝条件に合わない値のまま保存しようとすると中止して理由を表示。取得後に条件が変わったときの動きは クリア／警告／何もしない から選択。ルックアップサジェスト v3.0.0 以上と併用すると入力中の候補にも適用（オンオフ可・オフでも［取得］の一覧は絞り込まれる）。ヒーロー＋UPDATE（v1.1.0：条件外は「確認して保存」も選べる／検査は今回入れた・書き換えた値だけ）＋課題3＋before/after 2枚＋レコード画面1枚＋保存時ガード1枚＋設定3枚（1文・初回・よくある例）＋場面ごと＋サジェスト連携 before/after 2枚＋インストール手順5＋プラン（無料/プレミアム）＋できないこと5＋FAQ10＋SecurityBox＋ShareButtons＋CTA。無料はルックアップ1つ・絞り込み1通り、複数ルックアップ/条件セット/対応表/サジェスト連携はプレミアム。ゲストスペース対応
│   │   │   ├── pdf-edit/
│   │   │   │   └── index.astro # 【プレミアムプラグイン】PDF編集アシスト 製品ページ（/plugins/pdf-edit/）— 添付PDFに押印画像・手書き・テキスト・図形9種（四角/角丸四角/円/三角/ひし形/マーカー/チェック/バツ/多角形）・矢印をその場で書き加えて保存。最大の差別化は墨消し＝指定範囲を塗ったうえでそのページを画像として焼き直し、元の描画命令・埋め込み画像・透明な文字レイヤーをファイルから物理削除する（白塗りでは元の文字が残り全文検索から露出しうる問題への答え）。保存は「同じフィールドに追加（原本と最新のみ／すべて残す）」と「上書き（ファイル名は元のまま・保存直前に確認）」。サブテーブル内添付・一覧からの編集・パスワード付きPDF（許可制）・添付ファイルプレビュー連携に対応。追記した文字は字形をアウトラインで焼き込む（どのビューアでも同じ見た目／生成PDFでは検索対象外）。ヒーロー＋課題3＋押印＋赤入れ＋テキスト＋墨消し2枚組＋保存＋一覧/プレビュー連携＋設定画面7枚（5タブ・長いタブは前後2枚）＋主な機能16＋プラン＋手順5＋注意13＋SecurityBox＋ShareButtons＋FAQ9＋CTA。2026-09-17 に v1.2.6 のアップデート情報（署名欄の複数登録＋別のサインを追加・スクショ4枚）へ差し替え、本文に「現場で、その場で署名をもらう」セクションを新設（サイン専用モードの常設説明＋署名欄の複数登録）。スマホ（モバイル版レコード詳細）対応・ゲストスペース対応
│   │   │   ├── view-control/
...
│   │   │   ├── related-enhancer/
│   │   │   │   └── index.astro # 【プレミアムプラグイン】関連レコード拡張製品ページ（/plugins/related-enhancer/）— レコード詳細の関連レコード一覧をカーソルAPI全件取得で集計（合計/平均/件数・条件AND/OR・複数並列・閾値色分け）＋非表示フィールド集計/サブテーブル展開集計/集計結果の書き込み保存＋見た目（ゼブラ/列幅クリップ/列表示非表示/ヘッダー色）＋検索/ソート/列フィルタ＋横断ボード＋横並び一覧の表示位置そろえ（v1.1.0）＋集計カードの表示位置（上/下・v1.1.0）。無料/プレミアム比較（無料1対象・F-04/05/06はプレミアム限定）、インストール手順、FAQ、SecurityBox。単品販売なし＝プレミアム年間サポーターに含む（CTAは /plugins/supporter/?plan=all）。モバイルは集計カード閲覧のみ。kintone完結
│   │   │   ├── lookup-sync/
...
│   │   ├── conditional-form-setvalue-05.png  # 条件分岐フォーム v2.6.0 設定画面の動作欄 — 値をセットする行ごとの「開いたとき:」（入力済みでも上書きする／空欄のときだけ入れる）（UPDATE ボックス）
│   │   ├── conditional-form-setvalue-06.png  # 条件分岐フォーム v2.6.0「適用する画面」で詳細を選んだときの注意書き（設定画面の使い方⑪）
│   │   ├── conditional-form-setvalue-07.png  # 条件分岐フォーム v2.6.0 編集画面を開いた時点で確度100%・アクション内容が入っている（案件管理アプリ・抜粋。UPDATE ボックス）
...
│   │   ├── sheet-edit-after-07.png         # シート編集 v1.5.2 スクショ — チェックボックス列の選択ポップアップ（選択肢「転記」「未選択」＋確定用 OK ボタン。製品ページ UPDATEボックス用）
...
│   │   ├── sticky-board-update-02.png      # 付箋ボード v1.1.0 — 一覧の会社名の前に付箋の件数マークが並び、件数バーの付箋に会社名・顧客No.のタグが付いた状態
│   │   ├── sticky-board-update-03.png      # 付箋ボード v1.1.0 設定画面 — ピンの形の選択・10色プリセット＋カラーピッカー・見え方のプレビュー・項目を検索して選ぶ欄
...
│   │   ├── file-export-picker-01.png       # 添付ファイル出力 ファイル選択画面（添付13点の縮小画像グリッド・4点選択中・v1.1.0）＋ .webp
│   │   ├── file-export-picker-02.png       # 添付ファイル出力 拡大表示（原寸・4/10・選択中ボタン・前後送り・v1.1.0）＋ .webp
│   │   ├── file-export-config-05.png       # 添付ファイル出力 設定画面「レコード詳細画面での個別選択」カード（v1.1.0）＋ .webp
...
│   │   ├── image-compress-banner.png / -banner.webp / -banner-800.webp / image-compress-icon.png / image-compress-before-after.png / image-compress-notify.png / image-compress-subtable.png / image-compress-config-01〜05.png（＋各 .webp） # 画像圧縮プラグインの一覧カード・OGP・ランキング用アイコン・製品ページ掲載スクショ
...
│   │   ├── form-connect-banner.png      # フォーム連携 for kintone 見出しバナー（OGP/グリッド/製品ページ共用・1200×630）＋ .webp / -800.webp
│   │   ├── form-connect-icon.png        # フォーム連携 アイコン（200×200・ランキング/Top3/製品ページヒーロー・プラグイン同梱アイコンから生成）＋ .webp
│   │   ├── form-connect-config-01.png   # フォーム連携 設定画面：列とフィールドの対応づけ表（6列中4列が自動で当たった状態）＋ .webp
│   │   ├── form-connect-config-03.png   # フォーム連携 設定画面：このプラグインが使うフィールド（回答ID等4つ）＋ .webp
│   │   ├── form-connect-config-04.png   # フォーム連携 設定画面：① Google 接続（接続方式の選択＋接続中のアカウント。掲載用にメールアドレスは example.com へ差し替え）＋ .webp
│   │   ├── form-connect-config-05.png   # フォーム連携 設定画面：② 連携フォームの回答シート（スプレッドシート・対象シート・表示名・初回の取り込み範囲）＋ .webp
│   │   ├── form-connect-screen-01.png    # フォーム連携 設定画面の全体（上帯の製品名＋バージョン／左の目次／中央の4タブ／右のサイドバー）。「② 連携フォーム」タブ。他製品の config-01 と同じ体裁＋ .webp
│   │   ├── form-connect-screen-02.png    # フォーム連携 設定画面の全体：「③ 動作」タブ（自動取り込み・プレビュー・1回あたりの上限・選択肢に無い値の扱い）＋ .webp
│   │   ├── form-connect-owncloud.png    # フォーム連携 設定画面：接続方式で「自社の Google Cloud を使う」を選んだ状態（クライアントID/シークレット/APIキーの入力欄）。/plugins/form-connect/google-cloud/ の手順6で使用。値はすべて見本＋ .webp
...
│   │   ├── annotation-banner.png        # 注釈アシスト for kintone 見出しバナー（OGP/グリッド/製品ページ共用・1200×630・gen-plugin-banner.mjs で生成）＋ .webp / -800.webp
│   │   ├── annotation-icon.png          # 注釈アシスト アイコン（200×200・ランキング/Top3/製品ページヒーロー）＋ .webp
│   │   ├── annotation-use-01.png        # 使用：常に表示する注釈（レコード詳細の空きに「対応のきまり」カード）＋ .webp
│   │   ├── annotation-use-02.png        # 使用：マウスを乗せたときに開く注釈（期限の隣の注意アイコン）＋ .webp
│   │   ├── annotation-use-03.png        # 使用：クリックしたときに開く注釈（詳細の隣の情報アイコン）＋ .webp
│   │   ├── annotation-use-04.png        # 使用：一覧画面のツールバーに置いた注釈＋ .webp
│   │   ├── annotation-en-01.png         # 使用：表示言語 English のレコード詳細（同じ注釈が英語で出る）＋ .webp
│   │   ├── annotation-config-01.png     # 設定①注釈の一覧（画面・貼り先・表示のしかた）＋ .webp
│   │   ├── annotation-config-02.png     # 設定②1件の設定（STEP1 中身を書く／STEP2 見せ方を決める・全体）＋ .webp
│   │   ├── annotation-config-03.png     # 設定③English の言語タブ（プレビューも英語に）＋ .webp
│   │   ├── annotation-config-04.png     # 設定④全体の設定（新しい注釈のひな形）＋ .webp
│   │   ├── annotation-config-05.png     # 設定⑤配置モード（本物のレコード画面で置き直す）＋ .webp
│   │   ├── annotation-config-06.png     # 設定⑥配置モードのプレビュー（利用者と同じ見え方）＋ .webp
│   │   ├── annotation-config-07.png     # 設定⑦標準ボタンを覆ったときの警告（赤い輪＋バーの文言）＋ .webp
│   │   ├── annotation-config-08.png     # 設定⑧表示する条件（レコードの値・見ている人・画面・日時）＋ .webp
│   │   ├── annotation-config-09.png     # 設定⑨ヘルプボタンの設定（表記・既定・色・形）＋ .webp
│   │   ├── annotation-use-05.png        # 使用：［ヘルプ］ボタンで注釈を出し入れする画面＋ .webp
...
│   │   ├── pdf-edit-config-areas.png     # 設定画面の全体（③編集ツール・署名欄の複数登録・右サイドバー／v1.2.6）＋ .webp
│   │   ├── pdf-edit-areas-list.png       # 設定画面：署名欄の一覧（お客様署名欄／弊社確認欄の2行＋署名欄を追加）＋ .webp
│   │   ├── pdf-edit-areas-preview.png    # 設定画面：見本PDFの上に2つの署名欄（選択中＝緑／未選択＝灰の破線）＋ .webp
│   │   ├── pdf-edit-sign-choose.png      # 編集画面：「どの署名欄に置きますか？」の選択ダイアログ＋ .webp
...
│   │   ├── input-template-09-login-user.png  # 製品ページ UPDATE ボックス（v1.2.0）：保存ダイアログで担当者＝ログインユーザー＋このレコードの値・部署＝優先する組織・作業者＝ログインユーザー・作業日＝本日から3日後・メモ＝空欄 ＋ .webp
│   │   ├── input-template-08-table-rules.png # 旧 UPDATE ボックス（v1.1.0・現在は未使用）：保存ダイアログの「列ごとの値の決め方」（作業日＝本日・開始日時＝本日の 09:30・期限＝翌月の末日）＋ .webp
...
│   │   ├── report-designer-output-04.png   # 帳票デザイナー 出力例④ 印刷プレビュー（実データ＋決裁欄の印影3つ・v1.3.0）
...
│   │   ├── report-designer-action-04.png   # 帳票デザイナー 動作④ ボタン方式のポップアップ（見出し・色分け・v1.3.0）
│   │   ├── report-designer-config-05.png   # 帳票デザイナー フィールド欄の検索ポップアップ（v1.2.0）
│   │   ├── report-designer-config-06.png   # 帳票デザイナー フィールドをまとめて配置ダイアログ（v1.2.0）
│   │   ├── report-designer-config-07.png   # 帳票デザイナー 配置済みの要素リスト（⚠付き・v1.2.0）
│   │   ├── report-designer-config-08.png   # 帳票デザイナー STEP5 帳票の選び方＝ボタン方式のライブプレビュー（v1.3.0）
...
│   │   ├── report-designer-config-18.png   # 帳票デザイナー 設定STEP4「PDFを添付フィールドへ自動保存」＋連番の設定（v1.5.0）
│   │   ├── report-designer-record-01.png   # 帳票デザイナー レコードの添付欄に連番つきの帳票PDFが3件並ぶ（v1.5.0）
...
│   │   ├── field-styler-v250-subtable-limit.png # フィールドスタイラー v2.5.0 — 明細の品名列で文字数超過の行にエラーが出ている編集画面
│   │   ├── field-styler-v250-line-limit.png     # フィールドスタイラー v2.5.0 — 対応メモの行数超過を入力欄の下で知らせている様子
│   │   ├── field-styler-v250-config.png         # フィールドスタイラー v2.5.0 — 設定画面全体（テーブル内の列に文字制限・文字種制限が並ぶ）
│   │   ├── input-assist-zoom-01.png        # 入力アシスト v2.1.0 — 明細表の狭い列で入力中の文字が欄からはみ出す様子（拡大入力の前）
│   │   ├── input-assist-zoom-02.png        # 入力アシスト v2.1.0 — 拡大入力のダイアログ（見出しに行番号・全文表示・文字数・Tab/Esc の案内）
...
│   │   ├── kw-conditional-form-v2.6.0.zip                        # 条件分岐フォーム for kintone 配布ファイル（最新・手動配置・v2.6.0＝値のセットを条件を満たした状態で画面を開いたときにも実行（空欄のみ／上書きを選択）・詳細画面を選んだときの注意書き）
...
│   │   ├── kw-input-template-v1.1.0.zip                          # 入力テンプレート for kintone 配布ファイル（旧版・2026-09-25＝「テンプレートにする」でテーブルの日付・時刻・日時を列ごとに「本日」「現在日時」などで保存／選択画面のプレビューに表示）
...
│   │   ├── kw-mail-assist-v1.4.0.zip                             # メールアシスト for kintone 配布ファイル（旧版・参考保管）
│   │   ├── kw-mail-assist-v1.6.0.zip                             # メールアシスト for kintone 配布ファイル（旧版・参考保管・v1.6.0 下書きファイル（.eml）方式＝書式つき本文・添付ファイル対応／一斉送信でレコードごとに1通／条件に「操作している人」（組織・グループ・ユーザー・アプリ管理権限）追加・上限5→8／テンプレ編集中の背景クリックで内容が失われる不具合を修正／{ でフィールド候補／Webメーラーで2通目以降が開けない場合の通知。**公開当日にセルフレビューで見つけた不具合8件を直した zip に差し替え済み**＝レコードごとに1通でCC/BCCが空／同経路で送信履歴が残らない／日付書式の計算フィールドで大小比較ができない／メールアドレス取得の失敗が固定化する／組織一覧が取れないとき子組織展開が消える／消えた添付フィールドを黙って落とす／範囲外の文字参照で組み立てが落ちる／表のセル内改行が単語をつなげる）
...
│   │   ├── kw-sheet-edit-v1.5.1.zip                      # シート編集 for kintone 配布ファイル（旧版・参考保管）
│   │   ├── kw-sheet-edit-v1.5.2.zip                      # シート編集 for kintone 配布ファイル（最新・手動配置・v1.5.2 チェックボックス／複数選択セルの OK ボタンが見えず確定できなかったのを修正）
...
│   │   ├── kw-file-export-v1.1.0.zip                            # 添付ファイル出力 for kintone 配布ファイル（最新・手動配置・v1.1.0 レコード詳細画面で添付を縮小画像で並べ、ファイル単位で選んで出力／原寸の拡大表示／ZIP か1点ずつを選択）
...
│   │   └── Shiki.html          # Shiki（kintone計算式ジェネレーター）スタンドアロンHTMLアプリ（Astro管理外・62レシピ）
│   ├── drive-connect/
...
│   ├── form-connect/
│   │   └── callback.html       # kintoneプラグイン「フォーム連携（kw-form-connect）」の Google OAuth コールバック（静的HTML・サーバー処理なし・noindex）。ドライブ連携と OAuth クライアントは共用だが、コールバックは製品ごとに分けている（postMessage の source が kw-form-connect なので、どちらの設定画面が待っているかが一意に決まり、片方の変更がもう片方へ波及しない）。受け取った認可コードは保存も転送もせず、プラグイン設定画面へ postMessage で渡すだけ。宛先はプラグインが state に埋め込んだオリジンを cybozu.com / kintone.com 系に限定して照合してから決める（'*' にしない）。サイトの導線・sitemap には出さない
