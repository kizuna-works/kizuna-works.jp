...
│   │   ├── kw-mail-assist-v1.6.0.zip                             # メールアシスト for kintone 配布ファイル（旧版・参考保管・v1.6.0 下書きファイル（.eml）方式＝書式つき本文・添付ファイル対応／一斉送信でレコードごとに1通／条件に「操作している人」（組織・グループ・ユーザー・アプリ管理権限）追加・上限5→8／テンプレ編集中の背景クリックで内容が失われる不具合を修正／{ でフィールド候補／Webメーラーで2通目以降が開けない場合の通知）
│   │   ├── kw-mail-assist-v1.6.1.zip                             # メールアシスト for kintone 配布ファイル（最新・手動配置・v1.6.1 v1.6.0のセルフレビューで見つけた不具合8件を修正＝レコードごとに1通でCC/BCCが空／同経路で送信履歴が残らない／日付書式の計算フィールドで大小比較ができない／メールアドレス取得の失敗が固定化する／組織一覧が取れないとき子組織展開が消える／消えた添付フィールドを黙って落とす／範囲外の文字参照で組み立てが落ちる／表のセル内改行が単語をつなげる。本文長の目安を実測値の約1,200字に訂正）
