...
│   │   ├── kw-mail-assist-v1.6.0.zip                             # メールアシスト for kintone 配布ファイル（最新・手動配置・v1.6.0 下書きファイル（.eml）方式＝書式つき本文・添付ファイル対応／一斉送信でレコードごとに1通／条件に「操作している人」（組織・グループ・ユーザー・アプリ管理権限）追加・上限5→8／テンプレ編集中の背景クリックで内容が失われる不具合を修正／{ でフィールド候補／Webメーラーで2通目以降が開けない場合の通知）
